...
class Transformer(nn.Module):
    ...
    def __init__(
        ...
        self.control_proj = nn.Linear(control_dim, emb_dim)
    ...
    def forward(
        ...
        if cf_attention_mask is None:
            cf_attention_mask = control_feats.new_ones((control_feats.shape[0], control_feats.shape[1]), dtype=bool)
        ...
        pred_loc = loc[:, -1:] + pred_diff_loc_from_zero[:, :-1] # skip last predict (vn+1)
